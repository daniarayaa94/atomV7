...
class MailboxClass {
	...
	public function __construct($params) {
		...
		$this->setImapPath($params['imapPath']);
		$this->imapLogin = $params['login'];
		$this->imapPassword = $params['password'];
		$this->serverEncoding = strtoupper($serverEncoding);
		if( $params['attachmentsDir']) {
			if(!is_dir($params['attachmentsDir'])) {
				throw new Exception('Directory "' . $params['attachmentsDir'] . '" not found');
			}
			$this->attachmentsDir = rtrim(realpath($params['attachmentsDir']), '\\/');
		}
	}
